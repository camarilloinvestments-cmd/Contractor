...
// Collected credentials for one-time printing at the end of a demo seed.
const seededCreds: { email: string; password: string; role: string }[] = [];

// Resolve a demo password from an env var (>=8 chars) or generate a random one.
// NO hardcoded passwords: nothing predictable ships in source control.
function demoPassword(envKey: string): string {
...
async function main() {
  // Hard guard: demo/sample data must never seed automatically (e.g. on prod boot).
  if (process.env.ALLOW_DEMO_SEED !== 'true') {
    console.error(
      'Refusing to run demo seed. This script inserts sample accounts and data.\n' +
      'Set ALLOW_DEMO_SEED=true to explicitly opt in (never in production).',
    );
    process.exit(1);
  }

  console.log('Seeding DEMO database (sample accounts + data)...');

  // Admin user (visible)
  const adminPw = demoPassword('DEMO_ADMIN_PASSWORD');
  ...
  seededCreds.push({ email: 'admin@fibertrack.com', password: adminPw, role: 'ADMIN' });

  // Project Manager
  const pmPw = demoPassword('DEMO_PM_PASSWORD');
  const pmHash = await bcrypt.hash(pmPw, 12);
  await prisma.user.upsert({
  ...
  seededCreds.push({ email: 'pm@fibertrack.com', password: pmPw, role: 'PROJECT_MANAGER' });
  ...
  const w1Pw = demoPassword('DEMO_WORKER1_PASSWORD');
  const w1Hash = await bcrypt.hash(w1Pw, 12);
  await prisma.user.upsert({
  ...
  seededCreds.push({ email: 'carlos@ramirezsplicing.com', password: w1Pw, role: 'FIELD_WORKER' });

  const w2Pw = demoPassword('DEMO_WORKER2_PASSWORD');
  const w2Hash = await bcrypt.hash(w2Pw, 12);
  await prisma.user.upsert({
  ...
  seededCreds.push({ email: 'jake@fibertrack.com', password: w2Pw, role: 'FIELD_WORKER' });

  console.log('Seeding complete!');
  console.log('\n===== DEMO ACCOUNT CREDENTIALS (shown once) =====');
  console.log('NOTE: passwords apply to NEWLY created accounts only; existing accounts are left unchanged.');
  for (const c of seededCreds) {
