...
import { EstimatePDF } from '@/components/document-pdf';
...
import { buildEstimateSnapshot, createRevision, recordSend } from '@/lib/documents/snapshots';
import { writeAudit, requestMeta } from '@/lib/audit';

// Send (or resend) an estimate. Creates a NEW immutable revision + snapshot,
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const to = (body?.to as string) || estimate.primeContractor?.email || '';
    if (!to) {
    ...
    const revision = await createRevision('ESTIMATE', id, snapshot, user.id);

    const branding = await resolveBrandingForPdf();
    const pdfBuffer = await renderToBuffer(
      React.createElement(EstimatePDF, { estimate: estimate as any, branding }) as any
    );

    const profile = await getCompanyProfile();
    const templateKey = TEMPLATE_KEYS.ESTIMATE_NEW;
    const tmpl = await getTemplate(templateKey);
    const vars = mergeVariables(companyVariables(profile), estimateVariables(estimate));
    const subject = renderTemplate(tmpl?.subject ?? 'Estimate {{estimate_number}}', vars);
    const html = renderTemplate(tmpl?.bodyHtml ?? '<p>Estimate {{estimate_number}}</p>', vars);
    const text = renderTemplate(tmpl?.bodyText ?? 'Estimate {{estimate_number}}', vars);
    ...
    const result = await sendEmail({
      ...
      sentById: user.id,
      attachments: [
        { filename: `${estimate.estimateNumber}.pdf`, content: pdfBuffer as Buffer, contentType: 'application/pdf' },
      ],
    });
    ...
    await writeAudit({
      actor: { id: user.id, email: user.email, role: user.role },
      action: estimate.emailCount > 0 ? 'estimate.resend' : 'estimate.send',
      entityType: 'Estimate',
      entityId: estimate.id,
    ...
    return NextResponse.json({
      ...
      estimate: { id: updated.id, status: updated.status, emailCount: updated.emailCount },
    });
  } catch (err: any) {
    console.error('Estimate send error:', err?.message);
    return NextResponse.json({ ok: false, error: err?.message ?? 'Failed to send estimate' }, { status: 500 });
  }
}
