// Download the latest release package + manifest to isolated staging and verify
// it (checksum always; signature when a key is configured). A package that fails
// verification is deleted and never retained. ADMIN only.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!latest || !updateAvailable) return NextResponse.json({ error: 'No newer release to download.' }, { status: 400 });
    ...
    ensureDirs();
    // Download manifest first, then the package.
    ...
    if (!verification.ok) {
      ...
      await prisma.updateHistory.create({
        data: { action: 'DOWNLOAD', source: 'GITHUB', fromVersion: APP_VERSION, toVersion: manifest.version, commit: latest.commit,
    ...
    await prisma.updateSettings.update({ where: { id: UPDATE_SETTINGS_ID }, data: { latestAssetSha256: manifest.sha256 } });
    await prisma.updateHistory.create({
      data: { action: 'DOWNLOAD', source: 'GITHUB', fromVersion: APP_VERSION, toVersion: manifest.version, commit: latest.commit,
        result: 'SUCCESS', installedBy: session.user.email, message: `Downloaded & verified ${manifest.filename}`,
        logs: `checksum=OK signature=${verification.signatureOk === null ? 'n/a' : verification.signatureOk}`, finishedAt: now },
    });
    await writeAudit({
      actor: { id: session.user.id, email: session.user.email, role: session.user.role },
      action: 'system.update_download', entityType: 'UpdateSettings', entityId: s.id,
    ...
    await prisma.updateHistory.create({
      data: { action: 'DOWNLOAD', source: 'GITHUB', fromVersion: APP_VERSION, result: 'FAILED', installedBy: session.user.email, message: msg, finishedAt: now },
    });
